...
export default function DashboardClient({ user }: { user: AppUser }) {
  ...
  useEffect(() => {
    ...
    const preferenceTimer = window.setTimeout(() => setQuizSoundEnabled(soundEnabled), 0);

    let cancelled = false;
    async function syncEntries() {
      try {
        const cloudEntries = await loadDailyEntries();
        const cloudDates = new Set(cloudEntries.map((entry) => entry.date));
        const pendingMigration = localEntries.filter((entry) => !cloudDates.has(entry.date));
        const migratedEntries: DailyEntry[] = [];
        for (const entry of pendingMigration) {
          migratedEntries.push(await saveDailyEntry(user.id, entry));
        }
        if (localEntries.length) window.localStorage.removeItem(STORAGE_KEY);

        const merged = new Map<string, DailyEntry>();
        [...migratedEntries, ...cloudEntries].forEach((entry) => merged.set(entry.date, entry));
        const next = [...merged.values()].sort((a, b) => b.date.localeCompare(a.date));
        if (!cancelled) {
          setSavedEntries(next);
          if (pendingMigration.length) {
            setSyncNotice("端末内の日記をクラウドへ移しました。");
            setSyncIssue(false);
          }
        }
      } catch (error) {
        if (!cancelled) {
          ...
          setSyncNotice(error instanceof Error ? error.message : "クラウドの日記を読み込めませんでした。");
          setSyncIssue(true);
        }
      } finally {
  ...
  async function saveToday() {
    ...
    setSaveError(null);
    try {
      const saved = await saveDailyEntry(user.id, activeEntry);
      setActiveEntry(saved);
      setSavedEntries((entries) => [saved, ...entries.filter((entry) => entry.date !== saved.date)]);
      setSavedNotice(true);
    } catch (error) {
      setSaveError(error instanceof Error ? error.message : "クラウドに保存できませんでした。");
    } finally {
      setSaving(false);
    }
  }
    ...
    <main>
      ...
      {(loadingEntries || syncNotice) && (
        <div className={`cloud-status ${loadingEntries ? "loading" : ""} ${syncIssue ? "error" : ""}`} role="status">
          <span>{loadingEntries ? "↻" : syncIssue ? "!" : "✓"}</span>
          <p>{loadingEntries ? "クラウドの日記を読み込んでいます…" : syncNotice}</p>
          {!loadingEntries && <button type="button" onClick={() => setSyncNotice(null)} aria-label="Close cloud status">×</button>}
        </div>
      )}
      ...
      {screen === "history" && (
        <HistoryScreen entries={savedEntries} onOpen={openEntry} onCreate={goToCreate} />
      )}
...
function Header({ user, screen, reviewCount, onNavigate, onCreate, onSignOut }: {
  user: AppUser;
  screen: Screen;
  reviewCount: number;
    ...
    <header className="site-header">
      <nav className="nav-shell" aria-label="Primary navigation">
        ...
        <div className="header-actions">
          ...
          <button className="sign-out-button" type="button" onClick={onSignOut}>Log out</button>
        </div>
      </nav>
    </header>
  );
