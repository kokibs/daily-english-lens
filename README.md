...
- 端末の英語音声による日記の読み上げ
- ホーム画面の1問復習と、日付を選んでその日だけを学べるReview画面（1周ごとの結果・全問再挑戦・間違えた問題だけの再挑戦に対応）
- 復習問題では写真に写る場所・食べ物・活動などの具体的な名詞を日本語のヒントに残し、覚える英語表現だけを穴埋めで出題
- 正解時の効果音と、保存されるサウンド設定
- 過去の日記を写真から振り返れるアーカイブ
- 保存した日の写真・日記・復習データを本人が削除できる機能
